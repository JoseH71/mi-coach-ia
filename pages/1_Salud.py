...
st.set_page_config(
    page_title="Coach IA de Readiness v2.9.16",
    page_icon="🧠",
    layout="wide"
)
...
def calcular_ier(hrv_gap, rhr_gap, hrv_trend):
    ier = (hrv_gap * 2.0) + (rhr_gap * 1.0)
    if hrv_trend < -0.3: 
        ier -= 10
    if hrv_gap > 0 and rhr_gap > 0: 
        ier += 10
    return min(100, max(0, int(ier)))
...
def _score_hrv(df_including_today, hrv_hoy, historic_baseline_df):
    ...
    return score, points, verdict, alerts, breakdown, is_declining

def get_readiness_analysis_v16(selected_date, manual_context, df):
    ...
    score_h, hrv_points, hrv_verdict, alerts_h, breakdown_h, is_hrv_declining = _score_hrv(df_including_today, hrv_hoy, historic_baseline_df)
    ier_score = None
    hrv_baseline_rec = baselines.get('recovery', {}).get('hrv')
    rhr_baseline_rec = baselines.get('recovery', {}).get('restingHR')
    hrv_gap, rhr_gap, hrv_trend = 0, 0, 0
    ...
    if pd.notna(hrv_ma7_today) and pd.notna(hrv_ma7_3days_ago) and hrv_ma7_3days_ago > 0:
        hrv_trend = (hrv_ma7_today / hrv_ma7_3days_ago) - 1
    ier_score = calcular_ier(hrv_gap, rhr_gap, hrv_trend)
    ...
    if len(df_including_today) >= 28:
        ...
        if pd.notna(hrv_7d_mean) and pd.notna(hrv_28d_mean) and pd.notna(hrv_28d_std):
            if hrv_7d_mean < (hrv_28d_mean - hrv_28d_std):
                alerta = "🚨 ALERTA LUISMA: HRV 7D roza tu línea roja (-1 SD). Riesgo de sobreentrenamiento en 3-5 días"
                alerts.append(alerta)
            elif hrv_7d_mean > (hrv_28d_mean + hrv_28d_std):
                alerta = "✅ Supercompensación detectada. Hoy tolera carga +15%"
                alerts.append(alerta)
    ...
    return {"verdict": verdict_text, "readiness_score": score, "alerts": alerts, "score_breakdown": breakdown, "metrics": { "VFC (HRV)": hrv_hoy, "FC Reposo": rhr_hoy, "Puntuación Sueño": sleep_score_hoy }, "verdicts": {"hrv": hrv_verdict, "rhr": rhr_verdict, "sleep": sleep_verdict}, "points": {"hrv": hrv_points, "rhr": rhr_points, "sleep": sleep_points}, "baselines": baselines, "load_metrics": {"ctl": ctl_ayer, "atl": atl_ayer, "tsb": tsb_ayer}, "manual_context": manual_context, "ier_score": ier_score }
...
def generate_health_summary_range(start_date, end_date, df_full):
    ...
    while current_date <= end_date:
        analysis = get_readiness_analysis_v16(current_date, [], df_full)
...
def display_comparative_dashboard(readiness_score, ier_score):
    ...
    ier_color = "#d9534f" if ier_score <= 40 else "#f0ad4e" if ier_score <= 70 else "#5cb85c"
    ier_emoji = "🔴" if ier_score <= 40 else "🟡" if ier_score <= 70 else "🟢"
    ...
    <div class="dashboard-container">
        <div class="row">
            ...
            <div class="col-md-6 dashboard-col" style="padding-left: 20px;">
                <h3>{ier_emoji} Índice Estrés-Recup.</h3>
                <p>Detector de supercompensación</p>
                <div class="score" style="color: {ier_color};">{ier_score} <span style="font-size: 0.5em;">/ 100</span></div>
            </div>
        </div>
...
def generate_coaching_summary(analysis, df, selected_date):
    ...
    ier_score = analysis.get('ier_score', 0)
    ...
    if readiness_score >= 80:
        estado = "✅ VERDE - Sistema recuperado"
    elif readiness_score >= 60:
        estado = "⚠️ AMARILLO - Recuperación parcial"
    else:
        estado = "🚫 ROJO - Señales de fatiga"
    hrv_text = "N/A"
    ...
    hrv_trend_text, rhr_trend_text = "↔️ estable", "↔️ estable"
    if len(df) >= 28:
        hrv_7d_mean = df['hrv'].tail(7).mean()
        hrv_28d_data = df['hrv'].tail(28)
        hrv_28d_mean = hrv_28d_data.mean()
        hrv_28d_std = hrv_28d_data.std()
        if pd.notna(hrv_7d_mean) and pd.notna(hrv_28d_mean) and pd.notna(hrv_28d_std) and hrv_28d_std > 0:
            z_score = (hrv_7d_mean - hrv_28d_mean) / hrv_28d_std
            if z_score > 0.75: hrv_trend_text = f"↗️ mejorando (Z:{z_score: .2f})"
            elif z_score < -0.75: hrv_trend_text = f"↘️ empeorando (Z:{z_score: .2f})"
            else: hrv_trend_text = f"↔️ estable (Z:{z_score: .2f})"
        rhr_7d_mean = df['restingHR'].tail(7).mean()
        rhr_28d_mean = df['restingHR'].tail(28).mean()
        if pd.notna(rhr_7d_mean) and pd.notna(rhr_28d_mean):
            if rhr_7d_mean > rhr_28d_mean + 1: rhr_trend_text = "↗️ subiendo"
            elif rhr_7d_mean < rhr_28d_mean - 1: rhr_trend_text = "↘️ bajando"
    if readiness_score >= 80 and ier_score >= 71:
        ...
        patron = "⚠️ Base sólida con estrés agudo: Motor listo, pero sistema nervioso fatigado."
        plan = "Mantener volumen, pero guiar por Sensaciones (RPE) o FC. No forzar watios."
    elif readiness_score >= 60:
...
**RHR:** {rhr_text} - Tendencia: {rhr_trend_text}
**Patrón:** {patron}
**Plan:** {plan}
"""
    return summary
...
# --- INTERFAZ PRINCIPAL ---
st.title("🧠 Coach IA de Readiness v2.9.16")
...
baseline_types = {'recovery': 'De Recuperación', 'chronic': 'Crónica (28d)', 'historic': 'Histórica (60d)'} # LÍNEA CORREGIDA

if selected_date:
    df_full = get_wellness_data(selected_date - timedelta(days=90), selected_date)
    analysis = get_readiness_analysis_v16(selected_date, context_options, df_full)
    ...
    if "error" in analysis:
        ...
        with tab1:
            ...
            display_comparative_dashboard(analysis.get('readiness_score', 0), analysis.get('ier_score', 0))
            ...
            with st.expander("📋 Resumen Antiguo para Copiar"):
                ...
                resumen_texto += f"**Índice Estrés Recuperativo (IER):** {analysis.get('ier_score', 'N/A')} / 100\n---\n"
