...
def format_duration(seconds):
    """Convierte segundos a un formato de Xh Ym."""
    ...
    if events:
        ...
        if workouts:
            ...
            for workout in workouts:
                activity_date = datetime.fromisoformat(workout.get('start_date_local')).strftime('%A, %d de %B')
                activity_name = workout.get('name', 'Entrenamiento sin nombre')
                
                # --- Lógica de extracción usando los campos correctos ---
                description = workout.get("description", "")
                
                # Usamos los campos que has identificado
                duration_sec = workout.get('moving_time', 0)
                tss = workout.get('icu_training_load', 0)
                intensity_if = workout.get('icu_intensity', 0) # Esto es un %, ej: 53.85
                ...
                if intensity_if > 0:
                    # Convertimos la intensidad a IF decimal para el título
                ...
                with st.expander(expander_title):
                    ...
                    # Mostramos la descripción solo si no es "None" o vacía
