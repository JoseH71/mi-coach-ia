...
def fetch_data(url, headers):
    """Función genérica para obtener datos de la API de Intervals.icu."""
    try:
...
def calculate_training_metrics(activities_data):
    """Procesa las actividades de una semana para obtener métricas de entrenamiento."""
...
def get_weekly_analysis(end_date, planned_tss_list):
    """Función principal que orquesta el análisis de 4 semanas."""
...
if submit_button:
    ...
    if not df_analysis.empty:
        # --- SECCIÓN DE GRÁFICOS DE TENDENCIAS ELIMINADA SEGÚN SOLICITUD ---

        st.markdown("---")
        st.header("🗓️ Análisis Detallado por Semana")

        week_cols = st.columns(4)
        for i, week_name in enumerate(df_analysis.index[::-1]):
            with week_cols[i]:
                week_data = df_analysis.loc[week_name]
                st.subheader(week_name)

                st.metric(
                    label="TSS Realizado vs. Programado",
                    value=f"{week_data.get('TSS_Realizado', 0):.0f}",
                    delta=f"de {week_data.get('TSS_Programado', 0):.0f} programados"
                )
                ...
                with st.expander("Zonas de Potencia"):
                    ...
                    if sum(power_zones) > 0:
                        df_power = pd.DataFrame({
                            'Minutos': [s / 60 for s in power_zones],
                            'Zona': [f"Z{i+1}" for i in range(7)]
                        }).set_index('Zona')
                        st.bar_chart(df_power['Minutos'])
                    else:
                        st.caption("Sin datos de potencia.")
                
                with st.expander("Zonas de Frecuencia Cardíaca"):
                    hr_zones = week_data.get('HR_Zone_Times', [0]*7)
                    if sum(hr_zones) > 0:
                        df_hr = pd.DataFrame({
                            'Minutos': [s / 60 for s in hr_zones],
                            'Zona': [f"Z{i+1}" for i in range(7)]
                        }).set_index('Zona')
                        st.bar_chart(df_hr['Minutos'])
                    else:
                        st.caption("Sin datos de FC.")
                ...
                with st.expander("💡 Insights del Coach"):
                    ...
                    if ratio > 1.3:
                        st.warning(f"**Ratio ATL/CTL: {ratio:.2f}**. Carga muy alta, riesgo de fatiga excesiva.")
                    elif ratio > 1.1:
                        st.info(f"**Ratio ATL/CTL: {ratio:.2f}**. Buena carga productiva.")
                    else:
                        st.success(f"**Ratio ATL/CTL: {ratio:.2f}**. Carga de mantenimiento o recuperación.")
                    
                    ef = week_data.get('Eficiencia_Avg')
                    if ef:
                        st.info(f"**Eficiencia (NP/FC): {ef:.2f}**. Un aumento progresivo indica una mejor base aeróbica.")
                    
                    sleep = week_data.get('SleepScore_Avg')
                    if sleep and sleep < 75:
                        st.warning(f"**Puntuación de Sueño: {sleep:.1f}**. El descanso parece insuficiente para asimilar la carga.")

        st.markdown("---")
        st.header("📋 Tabla Resumen de Datos")
        
        # --- INICIO DE LA CORRECCIÓN DEL ERROR DE FORMATO ---
        # Seleccionamos solo las columnas numéricas para aplicar el formato
        numeric_cols = df_analysis.select_dtypes(include=np.number).columns
        # Aplicamos el formato solo a ese subconjunto de columnas
